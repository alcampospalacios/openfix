...
# Available models (routed through OpenClaw gateway)
AVAILABLE_MODELS = [
    # MiniMax
    ...
    # Anthropic
    {"id": "anthropic/claude-sonnet-4-5-20250514", "name": "Claude Sonnet 4.5", "provider": "anthropic"},
    ...
    {"id": "openrouter/qwen/qwen3-235b-a22b", "name": "Qwen3 235B", "provider": "openrouter"},
    {"id": "openrouter/meta-llama/llama-4-maverick", "name": "Llama 4 Maverick", "provider": "openrouter"},
    {"id": "openrouter/mistralai/mistral-large-latest", "name": "Mistral Large", "provider": "openrouter"},
]

# OpenClaw config file path (mounted volume)
OPENCLAW_CONFIG_PATH = Path("/app/openclaw/openclaw.json")
...
async def configure_model(model_config: ModelConfig, background_tasks: BackgroundTasks):
    ...
    # Update openclaw.json with the new model and provider API key
    _update_openclaw_config(model_config.model, model_config.api_key)

    # Restart openclaw gateway (to pick up new config) then agent
    background_tasks.add_task(restart_openclaw_and_agent)
    return {"status": "configured", "model": model_config.model, "openclaw_restarting": True, "agent_restarting": True}


def _update_openclaw_config(model: str, api_key: str | None):
    """Update openclaw.json with the selected model, and write the API key
    as an environment variable in openclaw.env (OpenClaw reads keys from env)."""
    provider = model.split("/")[0] if "/" in model else "openai"

    # 1. Update openclaw.json — only gateway + agents, no providers key
    try:
        config = {
            "gateway": {
                "port": 18789,
                "bind": "lan",
                "auth": {"mode": "token", "token": "openfix-token"}
            },
            "agents": {
                "defaults": {
                    "model": {"primary": model},
        ...
        print(f"OpenClaw config written: model={model}")
    except Exception as e:
        print(f"Error writing OpenClaw config: {e}")

    # 2. Write API key as env var in openclaw.env
    #    OpenClaw auto-reads PROVIDER_API_KEY from environment
    if api_key:
        try:
            env_path = OPENCLAW_CONFIG_PATH.parent / "openclaw.env"
            env_var = _provider_env_var(provider)
            lines = [
                "# Auto-generated by Openfix backend\n",
                f"{env_var}={api_key}\n",
            ]
            with open(env_path, "w") as f:
                f.writelines(lines)
            print(f"OpenClaw env written: {env_var}=***{api_key[-4:]}")
        except Exception as e:
            print(f"Error writing OpenClaw env: {e}")


# Map provider name → env var that OpenClaw expects
PROVIDER_ENV_VARS = {
...
def _provider_env_var(provider: str) -> str:
    return PROVIDER_ENV_VARS.get(provider, f"{provider.upper()}_API_KEY")


def restart_openclaw_and_agent():
    """Recreate openclaw (to pick up new env_file) then restart agent."""
    # OpenClaw needs force-recreate to re-read env_file with the new API key
    try:
        ...
        if result.returncode == 0:
            print("openclaw recreated successfully")
        else:
            print(f"openclaw recreate failed: {result.stderr}")
            # Fallback: simple restart (won't pick up new env, but at least restarts)
            subprocess.run(["docker", "restart", "openfix-openclaw-1"],
                           capture_output=True, timeout=30)
    except Exception as e:
        print(f"Error recreating openclaw: {e}")

    # Agent just needs a restart to reload config from backend DB
    try:
        result = subprocess.run(
        ...
        if result.returncode == 0:
            ...
            subprocess.run(["docker", "restart", "openfix-agent-1"],
                           capture_output=True, timeout=30)
    except Exception as e:
...
def restart_agent():
    """Restart only the agent (used by manual restart endpoint)."""
    restart_openclaw_and_agent()
